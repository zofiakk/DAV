...
This file contains the following functions:
    ...
    * make_subplots_d- creates the plot
"""
...
def main():
    ...
    make_subplots_d(avg_city, save=args.display,
                   fig_name="../images/fig14.png")

def make_subplots_d(data:pd.DataFrame, save:bool, fig_name:str):
    ...
    legend_handles = [plt.Line2D([0], [0], marker="_", markersize=20, markeredgewidth=2,
                                 color=color, label=label,
                                 linestyle='') for label, color in dict(reversed(list(new_palette2.items()))).items()]
    legend = plt.legend(handles=legend_handles, ncol=3,
                        bbox_to_anchor= (1.05, 1.1), title="Country" )
    ...
    g.fig.subplots_adjust(top = 0.85, left=0.085, right=0.9)
