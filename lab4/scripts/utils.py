...
import pandas as pd
from analyze_data import (get_average_year_city_temperature,
                          get_average_year_temperature)
...
def parse_args() -> argparse.ArgumentParser:
    ...
    if args.display == 1:
        print("Saving png files 'images' folder")
    elif args.display == 0:
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Function which loads and analyzes teh data from the csv file
    ...
    avg_city = get_average_year_city_temperature(filtered_data)

    return filtered_data, avg_temp, avg_city
